...
def upload(src, dest):
    ...
    cmd_adb_push = ['adb', 'push', src, dest]
    cmd_adb_sync = ['adb', 'push', '--sync', src, dest]

    if is_exist == '1' and is_empty == '1':
        # adb push
        print('Directory Music not exist')
        run_adb_cmd(cmd_adb_push)
    if is_exist == '0' and is_empty == '1':
        # adb push
        print('Directory Music exist,empty')
        run_adb_cmd(cmd_adb_push)
    if is_exist == '0' and is_empty == '0':
        # adb push --sync
        print('Directory Music exist,not empty')
