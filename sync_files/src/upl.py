...
def inst_log_arg(command, is_test_mode, path_logs_dir, name_model_recv_drive, name_sync_dir):
    if (is_test_mode):
        command[len(command) - 3] = '--log-file=' + path_logs_dir + '/' + name_model_recv_drive + '__' + name_sync_dir + '_TEST_MODE' +'.txt'
    else:
        command[len(command) - 3] = '--log-file=' + path_logs_dir + '/' + name_model_recv_drive + '__' + name_sync_dir + '.txt'
...
def upload_files(command, list_sync_dirs, name_model_recv_drive, path_logs_dir, is_test_mode):
    for dir in list_sync_dirs:
        ...
        inst_log_arg(command, is_test_mode, path_logs_dir, name_model_recv_drive, name_sync_dir)
...
def upload_vdi(command1, command2, full_pth_src_dir, root_pth_src_drive, full_path_dest_dir, name_model_recv_drive, path_logs_dir, is_test_mode):
    ...
    for i in list_vdi_img:
        ...
        if (not os.path.isfile(path_dest_vdi)):
            ...
            inst_log_arg(command1, is_test_mode, path_logs_dir, name_model_recv_drive, name_dir_vdi)
            out = bash_proc.run_cmd(command1)
        else:
            command2[len(command2) - 2] = i
            command2[len(command2) - 1] = path_dest_vdi
            inst_log_arg(command2, is_test_mode, path_logs_dir, name_model_recv_drive, name_dir_vdi)
